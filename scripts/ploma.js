...
var Ploma = (function () {
  ...
  self.Sketcher = function (canvas, textureCanvas1, textureCanvas2) {
    // Parameters
    var canvas = canvas;
    var textureCanvas1 = textureCanvas1;
    var textureCanvas2 = textureCanvas2;

    // State
    var ctx = canvas.getContext('2d');
    var textureImageData1 = textureCanvas1.getContext('2d').getImageData(0, 0, textureCanvas1.width, textureCanvas1.height).data;
    var textureImageData2 = textureCanvas2.getContext('2d').getImageData(0, 0, textureCanvas2.width, textureCanvas2.height).data;
    ...
    // filterPoint
    //
    // calls calculateFilteredPoints if enough points
    // are stored in curRawInputStroke
    //
    ...
    function calculateWidth(p) {
      ...
      if((p >= 0) && (p < 0.3)) {
        width = map(p, 0.3, 0.5, -2.5, -0.5);
      }
      if((p >= 0.3) && (p < 0.5)) {
        width = map(p, 0.3, 0.5, -0.5, 0.5);
      } 
      if((p >= 0.5) && (p < 0.95)) {
        width = map(p, 0.5, 0.95, 0.5, 0.65);
      }
      if(p >= 0.95) { // >1 Possible output from bezier
        width = 0.65;
      }
    ...
    function getNextTexturePixel(p) {
      ...
      // Pick texture to use
      var texture = (p <= 0.45) ? textureCanvas1 : textureCanvas2;
      var textureImageData = (p <= 0.45) ? textureImageData1 : textureImageData2;
